...
# Read the data
df = pd.read_csv('Stated_MNL_weight.csv')
df = df.drop(columns=['ResponseId'])
database=db.Database("Stated_MNL_weight",df)
...
biogeme = bio.BIOGEME(database,formulas,suggestScales=False)
biogeme.modelName = "MNL-Weight"
